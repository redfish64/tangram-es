...
static std::unordered_map<std::thread::id, RenderState*> s_renderStatesForThreads;

RenderState* RenderState::get() {
    auto thread = std::this_thread::get_id();
    auto renderState = s_renderStatesForThreads[thread];
    if (!renderState) {
        renderState = new RenderState();
        s_renderStatesForThreads[thread] = renderState;
    }
    return renderState;
}
...
void RenderState::activeTextureUnit(GLuint _unit) {
    // FIXME: current texture unit is changing, invalidate current texture binding:
...
int RenderState::nextAvailableTextureUnit() {
    if (m_textureUnit + 1 > Hardware::maxCombinedTextureUnits) {
